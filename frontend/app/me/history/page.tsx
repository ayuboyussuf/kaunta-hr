...
import AttendanceCalendar, { CalEntry, CalLeave } from "@/components/AttendanceCalendar";
...
export default function AttendanceHistoryPage() {
  ...
  const [leave, setLeave] = useState<CalLeave[]>([]);
  ...
  useEffect(() => {
    ...
    if (!token) return;
    Promise.all([
      api<{ attendance: AttendanceEntry[]; leave?: CalLeave[] }>("/api/employees/me/attendance", { token }),
      api<{ employee: Profile }>("/api/employees/me/profile", { token }),
    ])
      .then(([a, p]) => { setEntries(a.attendance); setLeave(a.leave ?? []); setProfile(p.employee); })
