...
function ScanInner() {
  ...
  if (!token) {
    return <p className="text-sm text-aproksi-red">This link is missing its workplace code.</p>;
  }
  if (!ready) {
    return <p className="text-sm text-aproksi-slate/60">Checking your session…</p>;
  }

  return (
    <div className="space-y-6">
      <h1 className="font-display text-3xl text-aproksi-ink">Clocking you in…</h1>
...
export default function ScanPage() {
  return (
    <main className="mx-auto max-w-md px-4 py-10">
      <Suspense fallback={<p className="text-sm text-aproksi-slate/60">Loading…</p>}>
        <ScanInner />
      </Suspense>
    </main>
  );
