...
/**
 * Payroll — human-gated review, approval, and payment preparation. Aproksi
...
const inputCls = "w-full rounded-lg border border-aproksi-mist bg-white px-3 py-2 text-sm outline-none focus:border-aproksi-ultra";
...
export default function PayrollPage() {
  ...
  if (loading) return <main className="bg-aproksi-stone grid place-items-center py-24"><Loader2 className="h-6 w-6 animate-spin text-aproksi-ultra" /></main>;
  ...
  return (
    <main className="bg-aproksi-stone min-h-screen">
      <div className="max-w-3xl mx-auto px-4 py-6">
        <div className="flex items-center justify-between mb-1">
          <h1 className="font-display text-3xl text-aproksi-ink">Payroll</h1>
          <Button variant="outline" onClick={() => setShowCreate((s) => !s)}><Plus className="h-4 w-4 mr-1" /> New run</Button>
        </div>
        <p className="text-sm text-aproksi-slate/60 mb-4">Calculated from attendance + rules. Nothing is paid — you approve, then pay manually.</p>

        {error && <div className="mb-4 rounded-lg border border-aproksi-red/30 bg-aproksi-red/5 px-4 py-3 text-sm text-aproksi-red">{error}</div>}
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 mb-4 -mx-1 px-1">
            {cycles.map((c) => (
              <button key={c.id} onClick={() => select(c.id)}
                className={`shrink-0 rounded-full px-4 py-1.5 text-sm border ${selId === c.id ? "border-aproksi-ultra bg-white text-aproksi-ink" : "border-aproksi-mist bg-white/60 text-aproksi-slate/70"}`}>
                {c.label}{c.locked && <Lock className="inline h-3 w-3 ml-1 text-aproksi-sage" />}
              </button>
            ))}
          </div>
        )}
        ...
        {!cycle ? (
          <div className={`${cardCls} p-10 text-center`}><Wallet className="h-8 w-8 text-aproksi-slate/30 mx-auto mb-3" /><p className="text-aproksi-slate/70">Create a run to begin.</p></div>
        ) : (
          <>
            {/* Status banner */}
            {locked ? (
              <div className="mb-4 rounded-lg border border-aproksi-sage/30 bg-aproksi-sage/10 px-4 py-3 text-sm text-aproksi-sage inline-flex items-center gap-2 w-full">
              ...
              <div className="mb-4 rounded-lg border border-aproksi-red/30 bg-aproksi-red/5 px-4 py-3 text-sm text-aproksi-red inline-flex items-center gap-2 w-full">
                <AlertTriangle className="h-4 w-4" /> {flaggedCount} item(s) need attention before you can approve.
              </div>
            ) : cycle.status === "open" ? (
              <div className="mb-4 rounded-lg border border-aproksi-mist bg-white px-4 py-3 text-sm text-aproksi-slate/70">Not run yet — press “Calculate draft”.</div>
            ) : (
              <div className="mb-4 rounded-lg border border-aproksi-amber/30 bg-aproksi-amber/10 px-4 py-3 text-sm text-aproksi-amber">Draft ready — review each person, then approve.</div>
            )}

            {/* Totals + actions */}
            <div className={`${cardCls} p-4 mb-4`}>
              <div className="flex flex-wrap items-center gap-4">
                <div><p className={labelCls}>Total to pay</p><p className="font-display text-2xl text-aproksi-ultra tabular-nums">{fmtKes(cycle.total_net ?? 0)}</p></div>
                <div><p className={labelCls}>Staff</p><p className="text-aproksi-ink text-lg">{cycle.employee_count ?? 0}</p></div>
            ...
            {lines.length === 0 ? (
              <div className={`${cardCls} p-8 text-center text-sm text-aproksi-slate/50`}>No lines yet — calculate the draft.</div>
            ) : (
              groups.map((g) => (
                <div key={g.name} className={`${cardCls} overflow-hidden mb-4`}>
                  <div className="flex items-center justify-between px-4 py-2.5 border-b border-aproksi-mist bg-aproksi-stone/50">
                    <span className="font-medium text-aproksi-ink text-sm">{g.name}</span>
                    <span className="text-sm tabular-nums text-aproksi-slate">{fmtKes(g.subtotal)}</span>
                  </div>
                  <div className="divide-y divide-aproksi-mist/60">
                    {g.lines.map((l) => {
                      ...
                      return (
                      <button key={l.id} onClick={() => setOpenLine(l)} className="w-full flex items-center justify-between gap-3 px-4 py-3 text-left min-h-[52px] active:bg-aproksi-stone/40">
                        <div className="min-w-0">
                          <p className="text-sm text-aproksi-ink truncate">{l.employee_name}{l.held && <span className="ml-1 text-xs text-aproksi-amber">(held)</span>}</p>
                          <p className="text-xs text-aproksi-slate/60">
                            {l.breakdown.days_present ?? 0}/{l.breakdown.expected_days ?? 0} days
                            {blocking && <span className="text-aproksi-red"> · needs attention</span>}
                            {info && !locked && <span className="text-aproksi-slate/50"> · review</span>}
                            {locked && <span className={l.payment_status === "paid" ? "text-aproksi-sage" : l.payment_status === "failed" ? "text-aproksi-red" : "text-aproksi-slate/50"}> · {l.payment_status === "not_yet" ? "not paid" : l.payment_status}</span>}
                          </p>
                        </div>
                        <div className="flex items-center gap-2 shrink-0">
                          {blocking && <AlertTriangle className="h-4 w-4 text-aproksi-red" />}
                          <span className="text-sm font-medium tabular-nums text-aproksi-ink">{fmtKes(l.net)}</span>
                          <ChevronRight className="h-4 w-4 text-aproksi-slate/40" />
                        </div>
                      </button>
                      );
                    })}
      ...
      {showApprove && cycle && (
        <Modal onClose={() => setShowApprove(false)} title="Approve payroll">
          <p className="text-sm text-aproksi-slate/70 mb-3">You&rsquo;re about to lock this run. Figures become final and cannot be edited.</p>
          <div className="rounded-lg bg-aproksi-stone p-3 mb-3 text-sm">
            <div className="flex justify-between"><span className="text-aproksi-slate/60">Total to pay</span><span className="font-medium text-aproksi-ink">{fmtKes(cycle.total_net ?? 0)}</span></div>
            <div className="flex justify-between"><span className="text-aproksi-slate/60">Employees</span><span className="font-medium text-aproksi-ink">{cycle.employee_count ?? 0}</span></div>
          </div>
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-end sm:items-center justify-center bg-black/40" onClick={onClose}>
      <div className={`${cardCls} w-full sm:max-w-md max-h-[85vh] overflow-y-auto rounded-b-none sm:rounded-b-[12px]`} onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-4 py-3 border-b border-aproksi-mist sticky top-0 bg-white">
          <div><p className="font-display text-lg text-aproksi-ink">{line.employee_name}</p><p className="text-xs text-aproksi-slate/60">{line.workplace_name}</p></div>
          <button onClick={onClose} className="p-1 text-aproksi-slate/50"><X className="h-5 w-5" /></button>
        </div>
        ...
        <div className="p-4 space-y-4 text-sm">
          ...
          {line.flags?.filter((f) => !f.resolved).map((f) => {
            ...
            return (
              <div key={f.code} className={`rounded-lg border px-3 py-2 ${blocks ? "border-aproksi-red/30 bg-aproksi-red/5" : "border-aproksi-amber/30 bg-aproksi-amber/10"}`}>
                <p className={`font-medium flex items-center gap-1 ${blocks ? "text-aproksi-red" : "text-aproksi-amber"}`}>
                  <AlertTriangle className="h-4 w-4" /> {blocks ? "Needs attention" : "For your info"}
                </p>
                <p className="text-aproksi-slate/80 mt-0.5">{f.message}</p>
                {/* Blocking flags (can't compute a number) can be acknowledged with a note. */}
                {!locked && blocks && (
                  <button onClick={() => onAction("resolve", f.code)} className="mt-1 text-xs text-aproksi-ultra hover:underline">Mark resolved (with a note)</button>
                )}
              </div>
            );
          })}
          ...
          <div>
            ...
            <p className="text-aproksi-ink">{b.days_present ?? 0} of {b.expected_days ?? 0} expected</p>
            {(b.present_dates?.length ?? 0) > 0 && <p className="text-xs text-aproksi-slate/60 mt-0.5">Present: {b.present_dates!.join(", ")}</p>}
            {(b.missing_dates?.length ?? 0) > 0 && <p className="text-xs text-aproksi-red mt-0.5">Missing: {b.missing_dates!.join(", ")}</p>}
          </div>

          {/* Gross */}
          <div className="flex justify-between"><span className="text-aproksi-slate/60">Gross · {b.gross_basis}</span><span className="text-aproksi-ink tabular-nums">{fmtKes(b.gross ?? 0)}</span></div>

          {/* Penalties */}
          {penalties.length > 0 && (
            <div>
              <p className={labelCls}>Penalties (from rules)</p>
              {penalties.map((d, i) => <div key={i} className="flex justify-between text-aproksi-slate/80"><span>{d.label}</span><span className="text-aproksi-red tabular-nums">−{fmtKes(d.amount)}</span></div>)}
            </div>
          )}
          {/* Absence (pro-rated) */}
          {absence.map((d, i) => <div key={`a${i}`} className="flex justify-between"><span className="text-aproksi-slate/80">{d.label}</span><span className="text-aproksi-red tabular-nums">−{fmtKes(d.amount)}</span></div>)}
          {/* Owner adjustments — each removable while unlocked */}
          {bonuses.map((d, i) => (
            <div key={`b${i}`} className="flex items-center justify-between gap-2">
              <span className="text-aproksi-slate/80">Bonus · {d.label}</span>
              <span className="flex items-center gap-2">
                <span className="text-aproksi-sage tabular-nums">+{fmtKes(d.amount)}</span>
                {!locked && d.adjustment_id && <button onClick={() => onRemove(line, d.adjustment_id!)} aria-label="Remove" className="text-aproksi-slate/40 hover:text-aproksi-red"><X className="h-3.5 w-3.5" /></button>}
              </span>
            </div>
          ))}
          {manual.map((d, i) => (
            <div key={`m${i}`} className="flex items-center justify-between gap-2">
              <span className="text-aproksi-slate/80">Deduction · {d.label}</span>
              <span className="flex items-center gap-2">
                <span className="text-aproksi-red tabular-nums">−{fmtKes(d.amount)}</span>
                {!locked && d.adjustment_id && <button onClick={() => onRemove(line, d.adjustment_id!)} aria-label="Remove" className="text-aproksi-slate/40 hover:text-aproksi-red"><X className="h-3.5 w-3.5" /></button>}
              </span>
            </div>
          ))}
          {b.override_net != null && (
            <div className="flex items-center justify-between gap-2">
              <span className="text-aproksi-amber">Net overridden</span>
              <span className="flex items-center gap-2">
                <span className="text-aproksi-amber tabular-nums">{fmtKes(b.override_net)}</span>
                {!locked && b.override_adjustment_id && <button onClick={() => onRemove(line, b.override_adjustment_id!)} aria-label="Remove override" className="text-aproksi-slate/40 hover:text-aproksi-red"><X className="h-3.5 w-3.5" /></button>}
              </span>
            </div>
          )}

          {/* Net */}
          <div className="flex justify-between border-t border-aproksi-mist pt-2"><span className="font-medium text-aproksi-ink">Net pay</span><span className="font-display text-lg text-aproksi-ultra tabular-nums">{fmtKes(line.net)}</span></div>
          ...
          {!locked ? (
            ...
            <div className="pt-1 space-y-2">
              {line.payment_ref && <p className="text-xs text-aproksi-slate/60">Payment ref: <span className="text-aproksi-ink">{line.payment_ref}</span></p>}
              ...
              {line.pdf_url && <a href={line.pdf_url} target="_blank" rel="noreferrer" className="inline-block text-sm text-aproksi-ultra hover:underline pt-1">Download payslip PDF</a>}
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 p-0 sm:p-4" onClick={onClose}>
      <div className={`${cardCls} w-full sm:max-w-sm p-4 rounded-b-none sm:rounded-[12px]`} onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-3"><h2 className="font-display text-lg text-aproksi-ink">{title}</h2><button onClick={onClose} className="p-1 text-aproksi-slate/50"><X className="h-5 w-5" /></button></div>
        {children}
      </div>
    </div>
  );
